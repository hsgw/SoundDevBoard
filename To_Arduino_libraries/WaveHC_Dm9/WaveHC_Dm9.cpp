...
uint8_t WaveHC::create(FatReader &f) {
  ...
  playingSamplesPerSec = dwSamplesPerSec;
...
void WaveHC::play(void) {
  ...
  OCR1A =  F_CPU / (playingSamplesPerSec);
...
void WaveHC::setSampleRate(uint32_t samplerate) {
  ...
  cli();
  while (TCNT0 != 0);
  playingSamplesPerSec = samplerate;
  OCR1A = F_CPU / samplerate;
  sei();
}
//------------------------------------------------------------------------------
/** Reset the player's sample rate.*/
void WaveHC::resetSampleRate() {
  cli();
  while (TCNT0 != 0);
  playingSamplesPerSec = dwSamplesPerSec;
  OCR1A = F_CPU / dwSamplesPerSec;
  sei();
}
//------------------------------------------------------------------------------
/** Stop the player. */
...
//------------------------------------------------------------------------------
/** Read Playing Position */

uint32_t WaveHC::getPos(void) {
  return fd->readPosition();
}

//------------------------------------------------------------------------------
/** Read Wave Stream byte Size */
uint32_t WaveHC::getStreamSize(void) {
